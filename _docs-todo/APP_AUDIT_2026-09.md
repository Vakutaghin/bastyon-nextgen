...
V34/X3-декодеры. Коммиты `1de0abe`…`66a8be9` (не запушены). Дальше — волна 2 (Tor честность, нужно решение Р1).
...
|---|---|---|
| vitest | 2128 passed / 1 skipped (177 файлов; часть тестов покрывает мёртвые модули — X11) → **2317/2317 (210 файлов) на 2026-09-14** | `node_modules/.bin/vitest run` |
...
| Rust | `cargo test --lib` 24/24 (ipfs) → **29/29 на 2026-09-14** (+ torrc, temp-пути), `cargo build` ok | `cd src-tauri && cargo test --lib` |
