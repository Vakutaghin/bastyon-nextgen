...
**Все пункты B/C/D закрыты и запушены в `o/main`:** аудит-фиксы — `29482de`,
subdomain-ссылки/`..` — `ff68f05`, pending-пост в posts-store (C7) и helper автора (D14) —
`c47d069`, подпись ecpair v3 + e2e-тест (D12) — `f82ab24`. Сводка правок и живая проверка
Kubo — в разделе G.
