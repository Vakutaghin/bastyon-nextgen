...
describe('parseBasytonLink', () => {
  ...
  it('extracts commentId from #comment- fragment', () => {
    const r = parseBasytonLink(`https://bastyon.com/post?s=${TXID}#comment-${COMMENT_ID}`)
    expect(r?.commentId).toBe(COMMENT_ID)
  })

  it('&c= takes priority over fragment', () => {
    const otherComment = 'c'.repeat(64)
    const r = parseBasytonLink(`bastyon://post?s=${TXID}&c=${COMMENT_ID}#comment-${otherComment}`)
    expect(r?.commentId).toBe(COMMENT_ID)
  })
  ...
  it('ignores short commentId fragment', () => {
    const r = parseBasytonLink(`bastyon://post?s=${TXID}#comment-${'a'.repeat(32)}`)
    expect(r?.commentId).toBeUndefined()
  })
  ...
  it('handles txid not as first param', () => {
    const r = parseBasytonLink(`https://bastyon.com/post?ref=share&s=${TXID}`)
    expect(r?.txid).toBe(TXID)
  })

  it('ignores unknown params gracefully', () => {
    const r = parseBasytonLink(`bastyon://post?s=${TXID}&ref=user123&share=1`)
    expect(r?.txid).toBe(TXID)
    expect(r?.isVideo).toBe(false)
  })
  ...
  it('rejects short txid (63 chars)', () => {
    expect(parseBasytonLink(`bastyon://post?s=${'a'.repeat(63)}`)).toBeNull()
  })

  it('rejects non-hex txid', () => {
    expect(parseBasytonLink(`bastyon://post?s=${'g'.repeat(64)}`)).toBeNull()
  })
...
describe('BASTYON_LINK_RE', () => {
  ...
  it('matches link with comment param', () => {
    const matches = matchAll(`bastyon://post?s=${TXID}&c=${COMMENT_ID}`)
    expect(matches).toHaveLength(1)
  })

  it('matches multiple links in text', () => {
    const txid2 = 'f'.repeat(64)
    const matches = matchAll(`Link1: bastyon://post?s=${TXID} Link2: bastyon://post?s=${txid2}`)
