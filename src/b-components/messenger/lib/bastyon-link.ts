 ...
 */
export const parseBasytonLink = (url: string): BastyonLinkTarget | null => {
  const lower = url.toLowerCase()
  if (!lower.startsWith('bastyon://') && !BASTYON_HOSTS.some((h) => lower.includes(h))) {
    return null
  }
  ...
  try {
    ...
    if (!url.startsWith('bastyon://') && !BASTYON_HOSTS.includes(parsed.hostname)) {
      return null
    }
    ...
    if (!commentId && parsed.hash) {
      const fragMatch = parsed.hash.match(/^#comment-([a-fA-F0-9]{64})$/i)
