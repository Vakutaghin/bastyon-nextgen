...
const isOptions = typeforce.maybe(typeforce.Object);
...
export class ECPair {
  constructor(__D, __Q, options) {
    ...
    if (__Q !== undefined) {
      const p = ecc.pointCompress(__Q, this.compressed);
...
export function fromPrivateKey(buffer, options) {
...
export function fromPublicKey(buffer, options) {
  typeforce(ecc.isPoint, buffer);
  typeforce(isOptions, options);
  return new ECPair(undefined, buffer, options);
}

export function fromWIF(wifString, network) {
...
export function makeRandom(options) {
  typeforce(isOptions, options);
  if (options === undefined) options = {};
