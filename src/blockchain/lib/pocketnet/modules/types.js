...
export function Signer(obj) {
  // ecpair v3 отдаёт publicKey как Uint8Array (не Node Buffer) и не имеет
  // getPublicKey(). Даунстрим (getSigningData + script_signature.encode) уже
  // нормализует Uint8Array→Buffer, поэтому и здесь принимаем Uint8Array —
  // иначе объектная форма txb.sign({...}) для контентных транзакций падает с
